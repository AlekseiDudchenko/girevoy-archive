...
for value in ['А.Е.Попова', 'АбдуллинР.Р.', 'Анасенко А.В.', 'Ананенко А.В.', 'Анасенко МС.В.']:
    print('|'.join(coach_names(value, merges, splits)))
`;
  const actual = execFileSync('python3', ['-c', script], execOptions).trim().split('\n');
  assert.deepEqual(actual, ['Попова А.Е.', 'Абдуллин Р.Р.', 'Анасенко Антон Владимирович', 'Анасенко Антон Владимирович', 'Анасенко Антон Владимирович']);
});
