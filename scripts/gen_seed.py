...
Регионы, клубы и федерации вставляются через INSERT OR IGNORE и берутся
обратно по имени: их id заняты демо-данными, а имена совпадают («Новосибирская
область» есть и там, и здесь). Всё остальное получает id со смещением BASE,
чтобы не столкнуться с демо-данными в одной базе.
"""
import json
import sys

BASE = 1000  # демо-данные занимают id до ~100
...
def main(path):
    ...
    # --- справочники: имена могут уже быть в базе от демо-данных
    regions, clubs = [], []
