...
import { links, renderIndex, renderCompetition, renderAthlete, renderResults } from './render.js';
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path.startsWith('/a/')) {
        const data = await q.getAthlete(db, decodeURIComponent(path.slice(3)));
        return data ? html(renderAthlete({ ...data, L })) : html(notFound(L), 404);
      }
