...
function coachProfileCard(body, roles) {
  ...
  const athletes = Number(attr('athletes') || 0);
  const results = Number(attr('results') || 0);
  const competitions = Number(attr('competitions') || 0);
  const years = Number(attr('years') || 0);
...
.profile-stats-four { grid-template-columns:repeat(4,minmax(0,1fr)); }
...
.coach-regions { display:flex; align-items:center; gap:.55rem; border-top:1px solid var(--rule); padding:.8rem .9rem; color:var(--ink-2); font-size:.9rem; }
.coach-regions .profile-icon { width:1.15rem; height:1.15rem; }
.coach-regions strong { color:var(--ink); }
...
@media (max-width:640px) {
  ...
  .profile-stats:not(.profile-stats-four) .profile-stat:last-child { grid-column:1 / -1; }
